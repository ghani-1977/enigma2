...
def streamrelayChecker(playref):
	...
	if '%3a//' not in playrefstring and playrefstring in whitelist.streamrelay:
		...
		playref = eServiceReference("%s%s%s:%s" % (playrefmod, url.replace(":", "%3a"), playrefstring.replace(":", "%3a"), ServiceReference(playref).getServiceName()))		print("[Whitelist_StreamRelay] Play service via streamrelay as it is whitelisted as such", playref.toString())
